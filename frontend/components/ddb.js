...
const stylesheetUrls = [
  "https://raw.githack.com/TeaWithLucas/DNDBeyond-DM-Screen/master/dm-screen.css",
];
...
var initalModules = {
  2080: function (module, __webpack_exports__, __webpack_require__) {
    ...
    for (key in character_rules_engine_lib_es) {
      if (
        typeof character_rules_engine_lib_es[key].getAbilities === "function"
      ) {
        crk = key;
        console.log("crk found: " + key);
      }
      if (
        typeof character_rules_engine_lib_es[key].getSenseTypeModifierKey ===
        "function"
      ) {
        ktl = key;
    ...
    for (key in Core) {
      if (
        typeof Core[key].WALK !== "undefined" &&
        typeof Core[key].SWIM !== "undefined" &&
        typeof Core[key].CLIMB !== "undefined" &&
        typeof Core[key].FLY !== "undefined" &&
        typeof Core[key].BURROW !== "undefined"
      ) {
        cmov = key;
    ...
    function getCharData(state) {
      ...
      function getSenseData(senses) {
        // finds returns the label
      ...
      return {
        ...
        situationalBonusSavingThrowsLookup:
          charf1.getSituationalBonusSavingThrowsLookup(state),
  ...
  $(linkUrlTarget).each(function (index, value) {
    ...
    if (charID != 0) {
      ...
      if (typeNode.hasClass("ddb-campaigns-detail-body-listing-active")) {
        let unassignedNode = $(value).parents(
          ".ddb-campaigns-detail-body-listing-unassigned-active"
        );
        ...
        typeNode.hasClass("ddb-campaigns-detail-body-listing-inactive")
      ) {
        type = "deactivated";
      }
      charactersData[charID] = {
        ...
        state: {
          appEnv: {
            ...
            dimensions: {
              sheet: { height: 0, width: 1200 },
              styleSizeType: 4,
              window: { height: 571, width: 1920 },
            },
            isMobile: false,
            isReadonly: false,
            redirect: undefined,
      ...
      for (let ruleID in optionalRules) {
        charactersData[charID].state.serviceData.definitionPool[
          optionalRules[ruleID].category
        ] = {
          accessTypeLookup: {},
          definitionLookup: {},
        };
    ...
    getJSONfromURLs(rulesUrls)
      .then((js) => {
        ...
        for (let id in charactersData) {
          ...
          charactersData[id].state.serviceData.ruleDataPool =
            rulesData.vehiclesRuleset;
        }
        ...
        js.forEach(function (charJSON, index) {
          if (isSuccessfulJSON(charJSON, index)) {
            ...
            Promise.all(promises).then(() => {
              var charData = moduleExport.getCharData(
                charactersData[charId].state
              );
              charactersData[charId].data = charData;
              updateElementData(charactersData[charId]);
              console.log(
                "Retrived Char Data for char " +
                  charId +
                  " aka " +
                  charactersData[charId].data.name
              );
...
function retriveCharacterRules(charId) {
  let promises = [];
  console.log(
    "Looking for optional rules for " + charactersData[charId].data.name
  );
  for (let ruleID in optionalRules) {
    if (
      ruleID in charactersData[charId].state.character &&
      charactersData[charId].state.character[ruleID].length > 0
    ) {
...
function retriveCharacterRule(charId, ruleID) {
  let url =
    gameCollectionUrl.prefix +
    optionalRules[ruleID].category +
    gameCollectionUrl.postfix;
        ...
        js.forEach(function (charJSON, index) {
          ...
          if (charJSON.success && charJSON.data.definitionData != undefined) {
            for (let data of charJSON.data.definitionData) {
              charactersData[charId].state.serviceData.definitionPool[
                optionalRules[ruleID].category
              ].definitionLookup[data.id] = data;
              charactersData[charId].state.serviceData.definitionPool[
                optionalRules[ruleID].category
              ].accessTypeLookup[data.id] = 1;
            }
          }
...
function insertControls(parent, campaignPrefix) {
  ...
  let autoUpdateLoaded = GM_getValue(
    campaignPrefix + "-autoUpdate",
    GM_getValue(scriptVarPrefix + "-autoUpdate", autoUpdateDefault)
  );
  let updateDurationLoaded = GM_getValue(
    campaignPrefix + "-updateDuration",
    GM_getValue(scriptVarPrefix + "-updateDuration", updateDurationDefault)
  );
...
function insertVisibilityControls(parent, campaignPrefix) {
  ...
  let showSavingThrows = controlsNode.find(
    'input[name ="gs-show-saving-throws"]'
  );
  ...
  // Loads ideally value set for this campaign, if not found it loads the last saved value otherwise it defaults
  let showAbilitiesLoaded = GM_getValue(
    campaignPrefix + "-showAbilities",
    GM_getValue(scriptVarPrefix + "-showAbilities", showAbilitiesDefault)
  );
  let showSavingThrowsLoaded = GM_getValue(
    campaignPrefix + "-showSavingThrows",
    GM_getValue(scriptVarPrefix + "-showSavingThrows", showSavingThrowsDefault)
  );
  let showSensesLoaded = GM_getValue(
    campaignPrefix + "-showSenses",
    GM_getValue(scriptVarPrefix + "-showSenses", showSensesDefault)
  );
  let showClassesLoaded = GM_getValue(
    campaignPrefix + "-showClasses",
    GM_getValue(scriptVarPrefix + "-showClasses", showClassesDefault)
  );
  let showResourcesLoaded = GM_getValue(
    campaignPrefix + "-showResources",
    GM_getValue(scriptVarPrefix + "-showResources", showResourcesDefault)
  );
...
function updateVisibility() {
  ...
  $(".gs-main-able")
    .parents(".gs-container")
    .toggle(abilities || saves);
  ...
  $(".gs-senses")
    .parents(".gs-container")
    .toggle(senses || classes || resources);
}
...
function insertCurrencies(parent, campaignPrefix) {
  ...
  let currenciesLoaded = GM_getValue(
    campaignPrefix + "-currencies",
    currenciesDefault
  );
  //console.log(currenciesLoaded);
  let container = parent.find(".gs-camp-currencies > .gs-container");

  let currencyAmount = parent.find(
    '.gs-camp-currencies > .gs-form-group input[name="gs-currency-amount"]'
  );
  let currencyType = parent.find(
    '.gs-camp-currencies > .gs-form-group select[name="gs-currency-type"]'
  );
  let currencyConfirm = parent.find(
    '.gs-camp-currencies > .gs-form-group button[name="gs-currency-confirm"]'
  );
  ...
  currencyConfirm.click(function () {
    ...
    if (updatedAmount != 0) {
      ...
      if (updatedAmount != undefined) {
        let currenciesUpdate = GM_getValue(
          campaignPrefix + "-currencies",
          currenciesDefault
        );
...
function updateLanguages(parent) {
  ...
  for (let id in charactersData) {
    ...
    if (character.type == "active") {
      let charLanguages = character.data.proficiencyGroups.find(function (e) {
        return e.label == "Languages";
      });
...
function updateElementData(character) {
  // function that builds the scraped data and renders it on the page.
...
function updateMainInfo(parent, character) {
  ...
  updateClasses(
    mainInfo,
    character.classes,
    character.spellCasterInfo.castingInfo
  );
...
function updatePassives(
  ...
  container
    .find(".gs-passivePerception .gs-passives-number")
    .html(passPerception);
  container
    .find(".gs-passiveInvestigation .gs-passives-number")
    .html(passInvestigation);
...
function updateSenses(parent, senses) {
  ...
  senses.forEach(function (item, index) {
    ...
    if (item.distance > 0) {
      ...
      curSense
        .find(".gs-additonal-sense-affix")
        .html(distanceUnit(item.distance));
    }
  });
  if (container.children().length < 1) {
...
function updateClasses(parent, classes, casting) {
  ...
  for (let i = 0; i < casting.modifiers.length; i++) {
    ...
    let charClass = classes.find(function (e) {
      return e.id == modifier.sources[0].id;
    });
...
function isSuccessfulJSON(js, name) {
  ...
  if (js.length < 1 || js.success == undefined) {
    ...
    console.warn(
      "JSON " + name + "'s retrieval was unsuccessful and is malformed"
    );
    return false;
